...
#modelTesting on cvs
model_version = "1.2.2"  
cv='models\greensitecropminmaxscaler.pkl'
cv2='models\greensitecropstandscaler.pkl'
cv3='models\greensitecropminmaxscaler.pkl'
def load_model(cv):
    try:
        with open(cv, 'rb') as model_file:
            if sklearn.__version__ == model_version:
                modelp = pickle.load(model_file)
            else:
                modelp = joblib.load(model_file)
        return modelp
